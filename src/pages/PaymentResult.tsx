...
export default function PaymentResultPage() {
  ...
  if (cancelled) {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
        <h1 className="text-xl uppercase tracking-widest text-[#e8e8e8]">Payment cancelled</h1>
        <p className="text-[#6b6b6b] mt-3 max-w-md text-sm">
          You cancelled the payment. No charge was made.
        </p>
        <Link
          to="/onboarding"
          className="mt-8 px-6 py-3 border border-[#00d4aa] text-[#00d4aa] uppercase text-xs hover:bg-[#00d4aa] hover:text-[#0a0a0b]"
        >
          GO BACK →
        </Link>
      </div>
  ...
  if (loading && !payload) {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center gap-4">
        ...
        <p className="font-mono text-[#6b6b6b] text-xs uppercase tracking-widest">Loading payment</p>
      </div>
    );
  }
  ...
  if (timedOut && status !== "CONFIRMED") {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center max-w-lg mx-auto">
  ...
  if (status === "CONFIRMED") {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
        ...
        <h1 className="mt-6 text-2xl uppercase tracking-widest text-[#e8e8e8]">Payment confirmed</h1>
        <p className="text-[#6b6b6b] mt-2">Your subscription is now active.</p>
        <p className="text-accent mt-4 text-sm">{planSummary}</p>
        <button
          type="button"
          onClick={() => navigate("/dashboard")}
          className="mt-10 px-8 py-3 border border-[#00d4aa] text-[#00d4aa] uppercase text-xs tracking-widest hover:bg-[#00d4aa] hover:text-[#0a0a0b]"
        >
          GO TO DASHBOARD →
        </button>
      </div>
  ...
  if (status === "FAILED") {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
        ...
        <p className="text-[#6b6b6b] mt-2 max-w-md text-sm">
  ...
  if (status === "EXPIRED") {
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
        <p className="text-[#6b6b6b] mt-6 text-sm uppercase">Payment expired</p>
        <p className="text-[#6b6b6b] mt-2 max-w-md text-xs">
  ...
  if (status === "PENDING") {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
        ...
        <p className="text-[#6b6b6b] text-sm mt-2 max-w-md">
        ...
        <button
          ...
          className="mt-8 px-4 py-2 border border-[#1f1f23] text-xs uppercase text-[#e8e8e8]"
        >
          CHECK STATUS
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] flex flex-col items-center justify-center p-8 font-mono text-center">
      ...
      <h2 className="mt-8 text-xl uppercase tracking-widest text-[#e8e8e8]">Waiting for payment</h2>
      <p className="text-[#6b6b6b] text-sm mt-2 max-w-lg">
        Complete your payment on Coinbase Commerce to continue.
      </p>
      <p className="mt-6 text-sm text-[#6b6b6b] uppercase tracking-widest">Charge code</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-8">
        ...
        <button
          type="button"
          onClick={() => void poll(chargeId)}
          className="px-6 py-3 border border-[#1f1f23] text-[#e8e8e8] uppercase text-xs"
        >
          CHECK STATUS
        </button>
      </div>
